...
let tokentime = uniCloud.getCurrentUserInfo().tokenExpired - Date.now()  //获取当前用户登录时间戳
if(tokentime<0){
	hostUserInfo = {}
}
...
const data = {
	...
	hasLogin: tokentime&&Object.keys(hostUserInfo).length != 0
}
...
export const mutations = {
	...
	async logout() {
		...
		uni.redirectTo({
			url: '/pages/self/self',///${pagesJson.uniIdRouter && pagesJson.uniIdRouter.loginPage ? pagesJson.uniIdRouter.loginPage: 
		});
